...
{		
	...
	for($i=1; $i<=$total; $i++)
	{
		...
		$accessionNumber= $_POST["accessionNumber$i"];
$result=$conn->query("select * from research where accessionNumber='$accessionNumber'");
		...
		$sql="INSERT INTO research(id,title,year,description,accessionNumber) VALUES('abhay','".$title."','".$year."','".$description."','".$accessionNumber."')";
		$sql = $conn->query($sql);	
...
<a href="addr.php" class="btn btn-large btn-info" id="back">&nbsp; Add Records</a>
</div>
<?php
if(isset($_POST['btn-gen-form']))
{
	...
	for($i=1; $i<=$_POST["no_of_rec"]; $i++) 
	{
        ...
        <td><input type="date" name="year<?php echo $i; ?>" placeholder="year" class='form-control' style="width:100%;height:30px;border-radius:5px;" required></td>
        ...
        <td><input type="text" name="accessionNumber<?php echo $i; ?>" placeholder="accession number" class='form-control' style="width:100%;height:30px;border-radius:5px;" required></td>
    ...
    <a href="research.php" class="btn btn-large btn-success" id="backn"> &nbsp; Back to index</a>
