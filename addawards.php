...
{		
	...
	for($i=1; $i<=$total; $i++)
	{
		...
		$sql="INSERT INTO awards(id,name,organisation,date,description) VALUES('abhay','".$name."','".$organisation."','".$date."','".$description."')";
		$sql = $conn->query($sql);		
	}
...
/*.p1{postion:relative;left:50%;top:2%;}
	...
	.container{position:absolute;
		top:20%;
		padding :2px;
		left:25%;max-height:60%;overflow:auto;}
		*{margin:0;}
...
<a href="adda.php" class="btn btn-large btn-info" id="back">&nbsp; Add Records</a>
</div>
<?php
if(isset($_POST['btn-gen-form']))
{
	...
	for($i=1; $i<=$_POST["no_of_rec"]; $i++) 
	{
        ...
        <tr>
        <td><?php echo $i; ?></td>
        <td><input type="text" name="name<?php echo $i; ?>" placeholder="name" class='form-control' style="width:100%;height:30px;border-radius:5px;" required></td>
        <td><input type="text" name="organisation<?php echo $i; ?>" placeholder="organisation" class='form-control' style="width:100%;height:30px;border-radius:5px;" required></td>
    ...
    <a href="awards.php" class="btn btn-large btn-success" id="backn"> &nbsp; Back to index</a>
