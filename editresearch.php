...
/*#logout{width:100px;position:absolute;bottom:10px;right:25px;}*/
</style>
...
<a href="addr.php" class="btn btn-large btn-info" id="addrecord"> &nbsp; Add Records</a>
</div>
<form method="post" action="updateresearch.php">
<table class='table table-bordered'>
<tr>
<td colspan=3 id="left" align:"center">research</td>
</tr>
<tr>
<td id="left">
Title
...
for($i=0; $i<$chkcount; $i++)
{
	...
	$res=$conn->query("SELECT * FROM research where accessionNumber = '$id'");
	...
	while($row=$res->fetch_array())
	{
		...
		<td>
		<input type="hidden" name="id[]" value="<?php echo $row['accessionNumber'];?>" />
		<input type="text" name="title[]" value="<?php echo $row['title'];?>" class="form-control" required>
		</td>
		<td>
		<input type="date" name="year[]" value="<?php echo $row['year'];?>" class="form-control" required>
		</td>
        <td>
...
<a href="research.php" class=" btn btn-danger" style="float:right";> &nbsp; cancel</a>
</td>
</tr>
</table>
</form>
